/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { ProductsCreateNestedManyWithoutCategoryOfWorksInput } from "./ProductsCreateNestedManyWithoutCategoryOfWorksInput";
import { Type } from "class-transformer";
import { WorkCreateNestedManyWithoutCategoryOfWorksInput } from "./WorkCreateNestedManyWithoutCategoryOfWorksInput";

@InputType()
class CategoryOfWorkCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => ProductsCreateNestedManyWithoutCategoryOfWorksInput,
  })
  @ValidateNested()
  @Type(() => ProductsCreateNestedManyWithoutCategoryOfWorksInput)
  @IsOptional()
  @Field(() => ProductsCreateNestedManyWithoutCategoryOfWorksInput, {
    nullable: true,
  })
  productsItems?: ProductsCreateNestedManyWithoutCategoryOfWorksInput;

  @ApiProperty({
    required: false,
    type: () => WorkCreateNestedManyWithoutCategoryOfWorksInput,
  })
  @ValidateNested()
  @Type(() => WorkCreateNestedManyWithoutCategoryOfWorksInput)
  @IsOptional()
  @Field(() => WorkCreateNestedManyWithoutCategoryOfWorksInput, {
    nullable: true,
  })
  works?: WorkCreateNestedManyWithoutCategoryOfWorksInput;
}

export { CategoryOfWorkCreateInput as CategoryOfWorkCreateInput };
